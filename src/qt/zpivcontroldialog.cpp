    ...
    for (const CMintMeta& mint : setMints) {
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zVPS.";
            else if (!mint.isSeedCorrect)
                strReason = "The zVPS seed used to mint this zVPS is not the same as currently hold in the wallet";
            else
