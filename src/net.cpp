...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        std::string strDesc = "VPS " + FormatFullVersion();
...
bool BindListenPort(const CService& addrBind, std::string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. VPS Core is probably already running."), addrBind.ToString());
        else
