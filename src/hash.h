...
/* ----------- Quark Hash ------------------------------------------------ */
template<typename T1>
inline uint256 lyra2rev3_hash(const T1 pbegin, const T1 pend)
{
    uint256 output;
    ...
    sph_blake256(&ctx_blake, (pbegin == pend ? pblank : static_cast<const void*>(&pbegin[0])), 80);
    sph_blake256_close (&ctx_blake, static_cast<void*>(hashA));	

    LYRA2_3(static_cast<void*>(hashB), 32, static_cast<const void*>(hashA), 32, static_cast<const void*>(hashA), 32, 1, 4, 4);
    
    sph_cubehash256_init(&ctx_cubehash);
    sph_cubehash256(&ctx_cubehash, static_cast<const void*>(hashB), 32);
    sph_cubehash256_close(&ctx_cubehash, static_cast<void*>(hashA));
    
    LYRA2_3(static_cast<void*>(hashB), 32, static_cast<const void*>(hashA), 32, static_cast<const void*>(hashA), 32, 1, 4, 4);
    
    sph_bmw256_init(&ctx_bmw);
    sph_bmw256(&ctx_bmw, static_cast<const void*>(hashB), 32);
    sph_bmw256_close(&ctx_bmw, static_cast<void*>(hashA));
    
    memcpy((void *) &output, hashA, 32);
    return output;
}
